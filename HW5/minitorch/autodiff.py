...
class FunctionBase:
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        return [
            (var, var.expand(der))
            for var, der in zip(inputs, cls_out)
...
def backpropagate(variable, deriv):
    ...
    topol_dict = {variable.unique_id: deriv}

    for var in topol_order:
        if var.is_leaf():
            var.accumulate_derivative(topol_dict[var.unique_id])
        else:
            cr_out = var.history.backprop_step(topol_dict[var.unique_id])
            for v, d in cr_out:
                if v.unique_id in topol_dict:
                    topol_dict[v.unique_id] += d
                else:
                    topol_dict[v.unique_id] = d
