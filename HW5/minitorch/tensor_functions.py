...
def make_tensor_backend(tensor_ops, is_cuda=False):
    ...
    class Backend:
        ...
        class Sigmoid(Function):
            @staticmethod
            def forward(ctx, a):
                sig_vals = sigmoid_map(a)
                ctx.save_for_backward(sig_vals)
                return sig_vals

            @staticmethod
            def backward(ctx, grad_output):
                sig_vals = ctx.saved_values
                grad_sigmoid = mul_zip(sig_vals, sig_vals)
                grad_sigmoid = add_zip(sig_vals, neg_map(grad_sigmoid))
                return mul_zip(grad_output, grad_sigmoid)
        ...
        class Exp(Function):
            ...
            def forward(ctx, a):
                exp_vals = exp_map(a)
                ctx.save_for_backward(exp_vals)
                return exp_vals

            @staticmethod
            def backward(ctx, grad_output):
                exp_vals = ctx.saved_values
                return mul_zip(grad_output, exp_vals)
