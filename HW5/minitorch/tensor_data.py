...
def index_to_position(index, strides):
    ...
    return int(sum([el * i for el, i in zip(index, strides)]))
...
def broadcast_index(big_index, big_shape, shape, out_index):
    ...
    out_index[:] = [
        big_index[i + len(big_shape) - len(shape)] if s > 1 else 0
...
def shape_broadcast(shape1, shape2):
    ...
    """
    shape_dif = abs(len(shape1) - len(shape2))
    if len(shape1) > len(shape2):
        shape2 = tuple([1] * shape_dif + list(shape2))
    else:
        shape1 = tuple([1] * shape_dif + list(shape1))

    if all([el1 == el2 or el1 == 1 or el2 == 1 for el1, el2 in zip(shape1, shape2)]):
        return tuple(max(el1, el2) for el1, el2 in zip(shape1, shape2))

    raise IndexingError("Broadcast Error")
...
class TensorData:
    ...
    def permute(self, *order):
        ...
        # TODO: Implement for Task 2.1. DONE
